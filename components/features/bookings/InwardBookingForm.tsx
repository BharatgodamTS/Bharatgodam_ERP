...
export default function InwardBookingForm() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData(prev => ({
      ...prev,
      [name]: name === 'weight' || name === 'storageDays' ? parseFloat(value) || 0 : value
    }));
  };
    ...
    if (formData.clientId && formData.warehouseId && formData.commodityId && formData.date && formData.weight > 0) {
      ...
      try {
        ...
        if (result.success) {
          ...
          alert('Inward booking created successfully!');
        } else {
          alert(result.message || 'Failed to create inward booking');
        }
      } catch (error) {
        console.error('Error creating inward booking:', error);
        alert('Failed to create inward booking. Please try again.');
      } finally {
  ...
  return (
    <div className="bg-white shadow-sm border border-slate-200 rounded-xl p-6">
      <h3 className="text-lg font-semibold text-slate-800 mb-4">Inward Booking</h3>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label htmlFor="storageDays" className="block text-sm font-medium text-slate-700 mb-1">Storage Days</label>
          <input
            type="number"
            id="storageDays"
            name="storageDays"
        ...
        >
          {isSubmitting ? 'Creating Booking...' : 'Create Inward Booking'}
        </button>
      </form>
    </div>
  );
